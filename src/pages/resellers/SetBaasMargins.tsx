...
export default function SetBaasMargins() {
  ...
  const [resellerMargins, setResellerMargins] = useState(
    resellers.map(reseller => {
      ...
      return {
        ...
        name: reseller.name,
        baasMargin: existingMargin?.baasMargin ?? 100, // Default 100% margin
      };
    })
  );
  
  const handleMarginChange = (resellerId: string, baasMargin: number) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Update all margins
    resellerMargins.forEach(({ resellerId, baasMargin }) => {
      const existingMargin = margins.find(m => m.resellerId === resellerId);
      setMargin(
        resellerId, 
        existingMargin?.vpsMargin ?? 100,
        baasMargin
      );
    });
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-secondary-900">Set BAAS Margins</h1>
        <p className="mt-2 text-secondary-600">
          Set profit margins for each reseller and sub-reseller for BAAS calculations.
        </p>
      </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-success-800">
                BAAS margins updated successfully
              </p>
            </div>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="card">
        <div className="border-b border-secondary-200 pb-5">
          <h2 className="text-lg font-medium text-secondary-800">BAAS Margins by Reseller</h2>
          <p className="mt-1 text-sm text-secondary-500">
            Set the profit margin percentage for backup-as-a-service (BAAS) pricing for each reseller.
          </p>
        </div>
